...
def _parse_window_minutes_list(raw: str | None) -> tuple[int, ...]:
    """15m BTC up/down only (5m lane disabled until re-enabled in engine)."""
    s = _strip(raw)
    if not s:
        return (15,)
    out: list[int] = []
    ...
    if tuple(out) != (15,):
        raise Prst1ConfigError(
            "PRST1_WINDOW_MINUTES must be exactly 15 for now (5m disabled). "
            f"Got {tuple(out)!r} from PRST1_WINDOW_MINUTES={raw!r}."
        )
    return (15,)
...
def _parse_entry_mode(raw: str | None) -> str:
    ...
    if s in ("EITHER_CHEAP", "TIGHT_BAND_UP"):
        return s
    raise Prst1ConfigError(
        "PRST1_ENTRY_MODE must be EITHER_CHEAP or TIGHT_BAND_UP "
        f"(got {raw!r})."
    )
